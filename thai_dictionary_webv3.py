...
custom_note = st.text_input("📝 คำอธิบายของคุณ:")
...
if query and user_name and query in memory:
    ...
    if own_entries:
        ...
        for idx, item in enumerate(own_entries):
            ...
            with col2:
                if st.button(f"✏️ แก้ไข {idx+1}"):
                    new_text = st.text_input(f"🔁 ป้อนคำอธิบายใหม่:", value=item["desc"], key=f"edit_{idx}")
            ...
            with col3:
                if st.button(f"🗑️ ลบ {idx+1}", key=f"delete_{idx}"):
                    if st.confirm(f"คุณแน่ใจหรือไม่ว่าต้องการลบคำอธิบายนี้?"):
