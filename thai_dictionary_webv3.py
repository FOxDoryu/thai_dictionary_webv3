...
def save_desc_callback():
    query = st.session_state.get("query_text", "").strip().lower()
    new_desc = st.session_state.get("new_desc", "").strip()
    ...
    if query and user_name and new_desc:
        ...
        st.warning("กรุณากรอกคำค้นหาและคำอธิบายให้ครบ")

def delete_desc_callback(idx):
    query = st.session_state.get("query_text", "").strip().lower()
    user_name = st.session_state.get("user", "")
    if query in memory and 0 <= idx < len(memory[query]):
        ...
        if item["name"] == user_name:
            ...
            st.error("คุณไม่สามารถลบคำอธิบายนี้ได้")
...
# ใช้ session_state เก็บ query_text แทนตรง input เพื่ออ้างอิงใน callback
if "query_text" not in st.session_state:
    st.session_state.query_text = ""

query = st.text_input("🔍 คำที่ค้นหา:", key="query_text")
...
if query:
    ...
    with col1:
        ...
        st.text_input("📝 คำอธิบาย:", key="new_desc")

        st.button("💾 บันทึก", on_click=save_desc_callback)

        # แสดงคำอธิบายพร้อมปุ่มลบของเจ้าของ
        if query in memory:
            st.markdown("---")
