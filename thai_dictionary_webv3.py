...
def main_app():
    ...
    if query:
        ...
        new_desc = st.text_input("📝 คำอธิบาย", value=st.session_state.new_desc, key="new_desc")
        ...
        if query in memory:
            ...
            for i, item in enumerate(memory[query]):
                ...
                if item['name'] == user_name:
                    ...
                    if st.button(f"🗑️ ลบ ({i+1})", key=del_key):
                        st.session_state[del_key] = True

            to_remove = [i for i in range(len(memory[query])) if st.session_state.get(f"del_{i}", False)]
            if to_remove:
                for index in sorted(to_remove, reverse=True):
                    memory[query].pop(index)
                    st.session_state.pop(f"del_{index}", None)
                if not memory[query]:
                    del memory[query]
                save_memory(memory)
