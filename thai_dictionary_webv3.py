...
# รีเซ็ต refresh param เพื่อป้องกัน rerun loop
if st.experimental_get_query_params().get("refresh") == ["true"]:
...
if query:
    ...
    if query in memory:
        ...
        for i, item in enumerate(memory[query]):
            if i not in st.session_state.edit_states:
                st.session_state.edit_states[i] = False

            if not st.session_state.edit_states[i]:
                ...
                if item['name'].strip().lower() == user_name.lower():
                    if col1.button("✏️ แก้ไข", key=f"btn_edit_{i}"):
                        st.session_state.edit_states[i] = True
                ...
                new_val = st.text_input(f"🔧 แก้ไขคำอธิบายที่ {i+1}:", value=item['desc'], key=f"edit_val_{i}")
