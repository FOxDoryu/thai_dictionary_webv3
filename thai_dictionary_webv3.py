    ...
    if query and user_name and query in memory:
        ...
        if own_entries:
            ...
            for idx, item in enumerate(own_entries):
                ...
                # แก้ไขแบบ inline
                new_text = st.text_area(f"แก้ไขคำอธิบายที่ {idx+1}:", value=item["desc"], key=f"edit_text_{idx}")
                if new_text.strip() != item["desc"]:
                    memory[query][memory[query].index(item)]["desc"] = new_text.strip()
                    save_memory(memory)
                    st.success(f"📝 แก้ไขคำอธิบายที่ {idx+1} สำเร็จแล้ว")
                    st.experimental_rerun()

                # ลบแบบมี checkbox ยืนยัน
                confirm_del = st.checkbox(f"ยืนยันลบคำอธิบายที่ {idx+1}", key=f"confirm_del_{idx}")
                if confirm_del:
                    ...
                    # ล้างค่า session_state ที่เกี่ยวข้องก่อน rerun
                    keys_to_delete = [k for k in st.session_state.keys() if k.startswith("confirm_del_") or k.startswith("edit_text_")]
                    for k in keys_to_delete:
                        del st.session_state[k]

                    st.warning(f"❌ ลบคำอธิบายที่ {idx+1} เรียบร้อยแล้ว")
                    st.experimental_rerun()
