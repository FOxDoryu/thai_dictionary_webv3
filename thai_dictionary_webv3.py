...
def login():
    # กำหนดค่า default session state ถ้ายังไม่มี
    ...
    if not st.session_state.logged_in:
        ...
        new_username = st.text_input("ชื่อผู้ใช้ใหม่", key="signup_username")
        new_password = st.text_input("รหัสผ่านใหม่", type="password", key="signup_password")
        ...
        if signup_clicked:
            if new_username in users:
                ...
                return False, True  # not logged in, rerun needed
        ...
        if st.sidebar.button("ออกจากระบบ"):
            st.session_state.logged_in = False
            st.session_state.user = ""
            return False, True  # logged out, rerun needed
        return True, False
...
user_name = st.session_state.user  # ใช้ชื่อที่ล็อกอิน
...
if query:
    ...
    if query in memory:
        ...
        for i, item in enumerate(memory[query]):
            ...
            # แสดงปุ่มลบเฉพาะรายการที่เป็นของ user ที่ล็อกอิน
