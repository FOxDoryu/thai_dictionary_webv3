...
if query:
    ...
    if st.button("💾 บันทึกคำอธิบาย"):
        if query not in memory:
            memory[query] = []
        if custom_note.strip() != "" and custom_note not in memory[query]:
            memory[query].append(custom_note)
            save_memory(memory)
            st.success("✅ เพิ่มคำอธิบายเรียบร้อยแล้ว!")
        else:
            st.warning("⚠️ คำอธิบายซ้ำหรือว่างเปล่า")
    ...
    if st.button("🧼 เคลียร์คำอธิบายของคำนี้"):
        if query in memory:
            del memory[query]
            save_memory(memory)
            st.warning("🗑️ ลบคำอธิบายของคำนี้เรียบร้อยแล้ว")
        else:
            st.info("ℹ️ คำนี้ยังไม่มีคำอธิบายในฐานข้อมูล")

# แสดงคำอธิบายทั้งหมดที่เกี่ยวข้องกับคำค้น
if query:
    st.markdown("---")
    st.markdown("### 🗂️ คำอธิบายที่เกี่ยวข้องกับ “{}”".format(query))
    ...
    if related:
        ...
        st.info("🔍 ยังไม่มีคำอธิบายเกี่ยวกับคำนี้ในระบบ")
