...
def send_email(to, subject, body):
    ...
    sender = "youremail@gmail.com"  # แก้เป็นอีเมลจริงของคุณ
...
def register_page():
    ...
    if st.button("สมัครสมาชิก", key="register_btn"):
        if not name or not email or not password or not confirm:
            ...
            # ส่งอีเมลยืนยัน (ถ้าตั้งค่า Gmail API ถูกต้อง)
            try:
            ...
            # เปลี่ยนหน้าแล้วรอให้ main loop rerun
            st.session_state.page = "login"
...
def main_app():
    ...
    if query:
        ...
        if query in memory:
            ...
            # ต้องใช้ copy list เพื่อป้องกันปัญหา loop แล้ว pop index
            ...
            if to_remove_index is not None:
                memory[query].pop(to_remove_index)
                if not memory[query]:
                    del memory[query]
                save_memory(memory)
...
def app():
    ...
    # ควบคุม rerun ให้อยู่จุดเดียว ป้องกัน error
